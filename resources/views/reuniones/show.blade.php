...
<div class="max-w-4xl mx-auto mt-8 px-4">
    <!-- Botón de regreso -->
    <div class="mb-6">
        <a href="{{ route('reuniones.index') }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-800 font-medium transition">
    ...
    <!-- Tarjeta de detalles de la reunión -->
    ...
    <div class="bg-gray-50 p-6 rounded-xl shadow space-y-4">
        ...
        @forelse ($reunion->actividades->whereNull('deleted_at') as $actividad)
            <div class="bg-white p-4 rounded-lg shadow-md border-l-4 border-indigo-500">
                <h4 class="text-lg font-bold text-indigo-700">{{ $actividad->titulo }}</h4>
                <p class="text-sm text-gray-400 mt-2">
                    <span class="font-medium">Responsable:</span> {{ $actividad->responsable ?? 'Sin asignar' }}
                    <span class="ml-4 font-medium">Fecha límite:</span> {{ \Carbon\Carbon::parse($actividad->fecha_limite)->format('d/m/Y') }}
                </p>
